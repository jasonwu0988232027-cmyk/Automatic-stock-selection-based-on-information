...
st.set_page_config(page_title="台股全市場排行榜 - 指標版", layout="wide")
                    ...
                    if not t_df.empty:
                        ...
                        if turnover > 0:
                            final_results.append({
                                ...
                                "成交值指標": round(turnover, 2) # 將成交值賦值給指標欄位
                            })
                except: continue
        except: pass
...
# --- 主 UI ---
st.title("📊 台股成交值 Top 100 排行榜")
st.markdown("> **數據規範**：所有數值均已四捨五入至小數點第 2 位。")

if 'last_run' not in st.session_state: st.session_state.last_run = 0
time_diff = time.time() - st.session_state.last_run

if st.button("🚀 開始分析全市場資金強度", type="primary"):
    if time_diff < 300:
        st.error(f"🛑 系統冷卻中，請等待 {int(300 - time_diff)} 秒。")
    else:
        st.session_state.last_run = time.time()
        tickers = get_all_tickers()
        st.write(f"🔍 搜尋到 {len(tickers)} 隻股票，計算指標中...")
        ...
        if not df_final.empty:
            ...
            st.subheader("🏆 今日成交值 Top 100 指標排行榜")
            
            # 使用列表控制需要格式化為兩位小數的欄位
            format_cols = ["收盤價", "成交金額(億)", "成交值指標"]
            
            st.dataframe(
                top_100.style.format({col: "{:.2f}" for col in format_cols})
                            .background_gradient(subset=['成交值指標'], cmap='Oranges'), # 改用橘色漸層標示指標
